...
from apps.massage.models import PatientModel, ServiceModel, Account, ServiceTypeModel, SessionModel, PaymentModel
from apps.massage.forms.reception.payments import SingularPaymentCreateSerializer, WholePaymentCreateSerializer
from apps.massage.forms.reception.registration import PatientRegistrationForm, SessionForm
...
def register_booking_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            session.save()
            return redirect('massage_reception:main_screen')
    else:
        form = SessionForm()
...
def singular_payment_view(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            payment.save()
            return redirect('massage_reception:session-detailed', pk=pk)
    return redirect('massage_reception:session-detailed', pk=pk)
...
def add_new_patient(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            patient.save()
            return redirect('massage_reception:register-booking')  # Replace with your success URL
    else:
        return redirect('massage_reception:register-booking')
