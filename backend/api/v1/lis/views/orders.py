...
from api.v1.lis.serializers.orders import OrderedLabResearchFilterSerializer, \
    UpdateContainerCodeSerializer, ValidateStatusesSerializer
...
@swagger_auto_schema(tags=['lis'], methods=["patch"], request_body=ValidateStatusesSerializer)
@api_view(['patch'])
@permission_classes((IsAuthenticated,))
def validate_ordered_research_view(request, pk=None):
