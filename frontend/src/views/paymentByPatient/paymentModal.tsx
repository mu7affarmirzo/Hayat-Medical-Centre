...
const PaymentModal = ({ open, close, selectedReceipt }) => {
    const paymentStateKeeper = useLocalObservable(
        () => PaymentStateKeeper.instance
    );

    const { payForPatient } = paymentStateKeeper;
    const [paymentType, setPaymentType] = React.useState<string>('INCOME ');
    const [handleOpenChild, setHandleOpenChild] =
    ...
    const handlecheckboxchange = (id: string) => {
        ...
        if (!isAdded.length) {
            ...
            const indexOfService = services.findIndex(
    ...
    const proceedPayment = () => {
        payForPatient(transaction).then((res) => setHandleOpenChild(true));
    };
            ...
            {selectedReceipt && (
                <Box className={classes.modalWrapper} style={{ width: 1400 }}>
                    ...
                    <div className={classes.modalBody}>
                        <Box sx={{ flexGrow: 1 }}>
                            <Grid container spacing={2}>
                                <Grid xs={3}>
                                    <h4
                                        className={`${classes.formSubtitle} ${classes.mb10}`}
                                    >
                                        Форма оплаты
                                    </h4>
                                    <FormControl
                                ...
                                <Grid xs={9}>
                                    ...
                                    <Box className={classes.tableWrapper}>
                                        <table className={table.table}>
                                            ...
                                            {selectedReceipt.receipt_appointments[0].services.map(
                                                (item) => (
                                                    <tr>
                                                        <td
                                                            style={{
                                    ...
                                    <Box className={classes.flexContainer}>
                                        ...
                                        <div style={{ width: '100%' }}>
                                            ...
                                            <table
                                                className={`${table.table} ${classes.elipsis}`}
                                            >
                                                <thead
                    ...
                    <Modal
                        ...
                        onClose={() => setHandleOpenChild(false)}
                    >
                        <>
                            <Box
                            ...
                            >
                                ...
                                <Button
                                    onClick={() => setHandleOpenChild(false)}
                                >
                                    OK
                                </Button>
                            </Box>
